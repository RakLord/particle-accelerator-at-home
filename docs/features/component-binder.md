...
`DensePackingMultiplier` is `2^DensePackingLevel` from the Laboratory tree, capped at level 5 (×32). See `docs/features/laboratory.md`.
...
A "Binder full" notification fires on the first incineration of a session (see `docs/features/helper-notifications.md`). The Binder cell renders a "FULL" overlay while at capacity. Subsequent incinerations are silent — the player has been warned.
...
Binder is purchased from the inventory like any other Component (see `docs/features/inventory.md`). At placement, the player selects which Element the Binder binds to from a per-Binder picker — the same picker pattern as the Injector's Codex element selector.
...
From the Component cost catalog (see `docs/features/component-cost.md` for the formula):
...
This forces a meaningful split-grid decision on every Carbon-era run: how many Subjects do I route to income, and how many do I bank? With per-Element Token cost scaling (`docs/features/bonds-and-tokens.md`), banking the *first* Token of each Element is cheap; banking the third is a real grid commitment.
...
- `docs/features/bonds-and-tokens.md` — what banked reserves are spent on.
- `docs/features/inventory.md` — placement/purchase flow.
- `docs/features/component-cost.md` — cost formula.
- `docs/features/component-creation-and-balancing.md` — balancing checklist.
